...
def build_screener(df: pd.DataFrame, line_map: dict, upcoming_team_map: dict = None, def_ratings: dict = None, debug: bool = False) -> pd.DataFrame:
    """
    ...
    for player, pdf in df.groupby(player_col):
        for prop, line in line_map.items():
            ...
            # Predicted stat
            predicted = avg_last_10 * weighted_conf
